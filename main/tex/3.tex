...
\begin{itemize}
	\item $\lp{\enc, \dec}$ is a symmetric encryption scheme with  key space $\K'$ and provides IND-CPA security (see \cite{katz2014introduction} \textsection3.4.2) and \emph{random key robustness}, which states that if $k \neq k'$ are independent random keys, then $\dec(\enc(k,m), k')$ should fail with high probability. AES128-GCM satisfies both requirements.
